...
} from "./tag-value-loader.js";

/** A loader that takes an event and returns reactions */
export type ReactionsLoader = (
	event: NostrEvent,
	relays?: string[],
...
export function createReactionsLoader(
	pool: UpstreamPool,
	opts?: ReactionsLoaderOptions,
): ReactionsLoader {
	const request = wrapUpstreamPool(pool);

	const eventLoader = createTagValueLoader(request, "e", {
		...opts,
		kinds: [kinds.Reaction],
	});
	const addressableLoader = createTagValueLoader(request, "a", {
		...opts,
		kinds: [kinds.Reaction],
	});
